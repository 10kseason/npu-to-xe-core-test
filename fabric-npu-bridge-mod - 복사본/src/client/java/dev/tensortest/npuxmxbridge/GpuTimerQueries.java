...
final class GpuTimerQueries {
	...
	RenderTelemetrySnapshot snapshot() {
		return new RenderTelemetrySnapshot(lastFrameCpuMs, lastFrameGpuMs, lastUploadCpuMs, lastUploadGpuMs);
	}
	...
	private void poll() {
		pollQueue(pendingFrameSpans, true);
		pollQueue(pendingUploadSpans, false);
	}

	private void pollQueue(ArrayDeque<QuerySpan> queue, boolean frameSpan) {
		while (!queue.isEmpty()) {
